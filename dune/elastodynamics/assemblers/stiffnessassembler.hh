    ...
    public:
	  ...
	  void assemble(LocalMatrix& localMatrix, LocalView& localView) {
        ...
        for(const auto& quadPoint : quadRule) {
          ...
          std::vector<FieldMatrix<double, 1, dimworld>> referenceGradients(localFE.size());
		  ...
		  std::vector<std::array<SymmetricTensor<dim>, dimworld>> strain(localFE.size());
		  for( int i=0; i<localFE.size(); i++) {
		    for( int k=0; k<dim; k++ ) {
		      Dune::FieldMatrix<double, dimworld, dimworld> deformationGradient(0);
		      deformationGradient[k] = gradients[i];
		      computeStrain(deformationGradient, strain[i][k]);
		    }            
          }

          Dune::Elastodynamics::HookeTensor<dimworld> hookeTensor(E_, nu_);
